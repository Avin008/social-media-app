...
const UserCard = ({
  ...
  const { _id: userId, token } = useAuthStore((store) => store);

  const [toggleUpdateProfileCard, setToggleUpdateProfileCard] = useState(false);
  ...
  return (
    <div className="flex h-40 items-center justify-between rounded-md border border-gray-600 bg-[#282C37] px-4">
      <div className="flex items-center gap-3">
        <Avatar image={userData?.profilePic} height="80px" width="80px" />
      ...
      {toggleUpdateProfileCard && (
        <div className="fixed bottom-0 left-0 right-0 top-0 z-50 flex items-center justify-center bg-black/20">
          <UpdateUserProfileCard
            userData={userData}
            toggleProfileCardHandler={toggleProfileCardHandler}
          />
        </div>
      )}
    </div>
